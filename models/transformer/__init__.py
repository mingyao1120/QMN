...
class DualTransformer(nn.Module):
    ...
    def forward(self, src1, src_mask1, src2, src_mask2, decoding, enc_out=None, gauss_weight=None, need_weight=False):
        ...
        if decoding == 1: # 
            if enc_out is None:
                enc_out, _ = self.decoder2(None, None, src2, src_mask2) # 对文本依赖进行建模
            out, weight = self.decoder1(enc_out, src_mask2, src1, src_mask1) # 视频查文本
        elif decoding == 2:
            if enc_out is None:
        ...
        enc_out_v, _ = self.decoder2(None, None, src1, src_mask1) # 对文本依赖进行建模

        if need_weight:
            return enc_out, out, weight,enc_out_v
        return enc_out, out,enc_out_v
